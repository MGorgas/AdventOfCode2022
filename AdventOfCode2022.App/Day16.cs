...
using static global::AdventOfCode2022.App.Day16;
...
public class Day16
{
	...
	public static void ProboscideaVolcanium()
	{
		...
		Func<Valve, int> calculateShortestPathToStart = graph.CreateGetShortestPathFunction(start);
		...
		List<Valve> valvesWithFlowRateGreaterZero = graph.Nodes.Where(n => n.FlowRate > 0).ToList();
		foreach (Valve fromValve in valvesWithFlowRateGreaterZero)
		{
			Func<Valve, int> calculateShortestPathToValve = graph.CreateGetShortestPathFunction(fromValve);

			foreach (Valve toValve in valvesWithFlowRateGreaterZero)
			{
				if (toValve == fromValve)
				{
					continue;
				}
	...
	public class Valve
	{
		...
		public override bool Equals(object? obj)
		{
			...
			{
				return Code.Equals(other.Code) && FlowRate == other.FlowRate;
			}
		...
		{
			return Code.GetHashCode() + FlowRate;
		}
	...
	public class ValveGraph : Graph<Valve>
	{
		...
		public Valve this[string code]
		{
			get
			{
				return AdjacencyList.Keys.Single(k => k.Code == code);
			}
		}
		...
		public static ValveGraph Parse(string[] lines)
		{
			...
			foreach (KeyValuePair<Valve, string[]> valveAndNeighbor in valveAndNeighbors)
			{
	...
	public class Edge<T>
	{
		...
		public override int GetHashCode()
		{
			return From.GetHashCode() + To.GetHashCode() + Weight.GetHashCode();
		}
		public Edge<T> Reverse()
		{
			return new Edge<T>(To, From, Weight);
		}
	}
	...
	public abstract class Graph<T>
	{
		...
		public List<T> this[T key]
		{
			get
			{
				return AdjacencyList[key].Select(e => e.To).ToList();
			}
		}
		...
		public IEnumerable<Edge<T>> Edges
		{
			get
			{
				return AdjacencyList.Values.SelectMany(v => v);
			}
		}
		...
		{
			AdjacencyList[node] = new HashSet<Edge<T>>();
		}
		...
		{
			foreach (KeyValuePair<T, HashSet<Edge<T>>> entry in AdjacencyList)
			{
				entry.Value.Clear();
			}
		}
		...
		{
			return AdjacencyList[from].First(e => e.To.Equals(to)).Weight;
		}
		...
		public void AddEdge(Edge<T> edge)
		{
			if (AdjacencyList.ContainsKey(edge.From) && AdjacencyList.ContainsKey(edge.To))
			{
				...
				AdjacencyList[edge.To].Add(edge.Reverse());
			}
		}
	}
	...
	public class Algorithms
	{
		...
		public static long Solve(ValveGraph graph, Valve startValve, int minutes, bool partTwo = false)
		{
			...
			else
			{
				...
				List<(string Path, long FlowRate)> result = paths.Select((p, i) => (Path: string.Concat(p.Skip(1).Select(v => v.Code)), FlowRate: GetPreasureRelease(graph, minutes, p))).ToList();
				...
				foreach (KeyValuePair<string, long> entry in pathLookup)
				{
					adjacentPaths.Remove(entry.Key);
					foreach (KeyValuePair<string, long> other in adjacentPaths.Where(p => !entry.Key.Chunk(2).Select(c => new string(c)).Any(c => p.Key.Contains(c))))
					{
		...
		private static IEnumerable<IEnumerable<Valve>> GetValidPaths(ValveGraph graph, IList<Valve> valves, Valve valve, int minutes)
		{
			...
			else
			{
				...
				List<Edge<Valve>> edges = graph.AdjacencyList[valve].Where(e => minutes - e.Weight > 0).Where(e => otherValves.Contains(e.To)).ToList();
				...
				if (edges.Any())
				{
					foreach (Edge<Valve> edge in edges)
					{
						...
						List<IEnumerable<Valve>> result = GetValidPaths(graph, otherValves, edge.To, minutes - edge.Weight).ToList();
...
public static partial class EnumerableExtensions
{
	...
	{
		return items.GetPermutations(items.Count());
	}
	...
	private static IEnumerable<IList<T>> GetPermutations<T>(this IList<T> items, int count)
	{
		...
		else
		{
			...
			{
				foreach (IList<T> permutation in items.GetPermutations(count - 1))
				{
					yield return permutation;
				}
				RotateRight(ref items, count);
			}
		}
	}
	...
	{
		return items.GetPermutations(startItem, items.Count());
	}
	...
	private static IEnumerable<IList<T>> GetPermutations<T>(this IList<T> items, T startItem, int count)
	{
		...
		else
		{
			for (int i = 0; i < count; i++)
			{
				foreach (IList<T> permutation in items.GetPermutations(startItem, count - 1))
				{
					yield return permutation;
				}
				RotateRight(ref items, count);
